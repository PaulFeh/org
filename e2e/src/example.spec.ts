...
test('navigate to add pet and submit form', async ({ page }) => {
  ...
  const [request] = await Promise.all([
    page.waitForRequest(request =>
      request.url().includes('/pet') && request.method() === 'POST'
...
test('pet details page displays pet info', async ({ page }) => {
  ...
  const firstPet = page.locator('li a').first();
  await firstPet.click();
  await expect(page).toHaveURL(/\/pet\//);
  await expect(page.locator('h2')).toContainText('Pet Details');
  await expect(page.locator('.pet-info-text')).toBeVisible();
